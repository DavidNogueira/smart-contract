var SubstratumCoin = artifacts.require("./SubstratumCoin.sol");
...
var Substratum_PER_ETHER = 6000000000;
var RECEIVE_Substratum_AMOUNT = SEND_ETHER * Substratum_PER_ETHER + ((SEND_ETHER * Substratum_PER_ETHER) / 5); // + 20% bonus
...
contract('MainFlow', function(accounts) {
  ...
  it("should put 1,000,000,000.000000 SubstratumCoin in the owner account", function() {
    return SubstratumCoin.deployed().then(function(instance) {
  ...
  it("Send 600,000,000.000000 SubstratumCoin to Crowdsale contract", function() {
    return SubstratumCoin.deployed().then(function(coin) {
  ...
  it("Buy 100,000,000 coins", function() {
    return Crowdsale.deployed().then(function(crowd) {
        ...
        console.log("Buyer balance: ", balance.valueOf(), " Substratum");
        assert.equal(balance.valueOf(), RECEIVE_Substratum_AMOUNT, RECEIVE_Substratum_AMOUNT + " wasn't in the first account");
     });
  });
  ...
  it("Try to reserve the payments {from: buyer}", function() {
    return SubstratumCoin.deployed().then(function(coin) {
      return coin.balanceOf.call(buyer).then(function(balance) {
        return Crowdsale.deployed().then(function(crowd) {
          console.log('Buyer Substratum: ' + balance.valueOf());
  ...
  it("Try to buy too more coins {from: buyer}", function() {
    return Crowdsale.deployed().then(function(crowd) {
       return crowd.sendTransaction({from: buyer, to: crowd.address, value: web3.toWei(CROWDSALE_CAP/Substratum_PER_ETHER+1, "ether")}).then(function(txn) {
          assert(false, "Throw was supposed to throw but didn't.");
       })
     }).catch(function(error) {
        console.log("Throw was happened. Test succeeded.");
  ...
  it("Buy 6,000 coins without bonus", function() {
    ...
    return Crowdsale.deployed().then(function(crowd) {
       return crowd.sendTransaction({from: buyer, to: crowd.address, value: web3.toWei(1, "ether")}).then(function(txn) {
        ...
        console.log("Buyer balance: ", balance.valueOf(), " Substratum");
        assert.equal(balance.valueOf(), RECEIVE_Substratum_AMOUNT + Substratum_PER_ETHER, RECEIVE_Substratum_AMOUNT + Substratum_PER_ETHER + " wasn't in the first account");
     });
  });
  ...
  it("Try to invoke backSubstratumCoinOwner {from: buyer}", function() {
    return Crowdsale.deployed().then(function(crowd) {
      return crowd.backSubstratumCoinOwner({from: buyer}).then(function() {
        assert(false, "Throw was supposed to throw but didn't.");
      }).catch(function(error) {
        console.log("Throw was happened. Test succeeded.");
      });
  ...
  it("Invoke backSubstratumCoinOwner {from: Crowdsale contract}", function() {
    return Crowdsale.deployed().then(function(crowd) {
      return crowd.backSubstratumCoinOwner().then(function() {
        return SubstratumCoin.deployed().then(function(coin) {
          return coin.owner.call().then(function(coinOwner) {
            console.log("SubstratumCoin owner was changed to: " + coinOwner);
            assert.equal(coinOwner, owner, "SubstratumCoin owner addresws must be equals to Crowdsale owner address");
          })              
        })
      }).catch(function(error) {
  ...
  it("Invoke backSubstratumCoinOwner one more time {from: Crowdsale contract}", function() {
    return Crowdsale.deployed().then(function(crowd) {
      return crowd.backSubstratumCoinOwner().then(function() {
        assert(false, "Throw was supposed to throw but didn't.");
      }).catch(function(error) {
